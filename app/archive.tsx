...
// import { getArchivedTasks } from './services/api'; // ← добавить в 
// import {
//   ActivityIndicator,
//   Alert,
//   Dimensions,
...
import { getArchivedTasks, getMe } from './services/api';
import { GetTaskInfo, GetUserInfo } from './services/types';

const { width } = Dimensions.get('window');

export default function ArchiveScreen() {
  const router = useRouter();
  const [archivedTasks, setArchivedTasks] = useState<GetTaskInfo[]>([]);
  ...
  // Загружаем архивные задачи
  const loadArchivedTasks = async () => {
    if (!user?.id) return;
    
    setLoading(true);
    try {
      const response = await getArchivedTasks(user.id);
      setArchivedTasks(response.data);
    } catch (error: any) {
      console.error('Load archived tasks error:', error);
      if (error.response?.status === 401) {
        router.replace('/login');
      } else {
        Alert.alert('Ошибка', 'Не удалось загрузить архив');
  ...
  const getTaskPoints = (task: GetTaskInfo) => {
    switch (task.priority) {
      case 3: return 15;
      case 2: return 10;
      case 1: return 5;
        ...
        >
          {archivedTasks.length === 0 ? (
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>Архив пуст</Text>
              <Text style={styles.emptySubtext}>
                Завершённые задачи будут автоматически попадать в архив
              </Text>
            </View>
          ) : (
            archivedTasks.map((task) => (
              ...
              >
                ...
                <View style={styles.taskInfo}>
                  <Text style={styles.taskTitle}>{task.title}</Text>
                  <Text style={styles.taskSubtitle}>
                    Завершена: {formatDate(task.completed_at || task.created_at)}
                  </Text>
                </View>
                <View style={styles.checkCircleDone}>
                  <Ionicons name="checkmark" size={18} color="#FF8DA1" />
